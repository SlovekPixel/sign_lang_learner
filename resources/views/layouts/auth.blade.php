    ...
    <header class="header-wrap">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-lg-2 col-md-2 col-3 logo-col">
                    <a class="logo text-center d-block" href="{{ route('profile') }}">
                        <img src="{{ asset('img/logo.png') }}" width="48" height="48" alt="Жесты">
                    </a>
                </div>
                <div class="col-lg-3 col-md-4 d-none d-md-block">
                    <div class="result-one">
                        <img src="{{ asset('img/icon.png') }}" width="32" height="32" alt="">
                ...
                <div class="col-lg-3 d-none d-lg-flex justify-content-end">
                    <div class="result-two">
                        ...
                        <span class="result-txt">Средний результат всех</span>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6 col-9 npdng">
                    <div class="mode-select">
                        <span class="mode-select__hint">Выбрать режим</span>
                        <select id="slct" name="slct" aria-label="Выбрать режим" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
                            <option selected disabled>{{ $page ?? '—' }}</option>
                            {{! $route = Request::route()->getName() }}
                            @if($route != 'profile')
                                <option value="{{ route('profile') }}">Обучение</option>
                            @endif
                            @if($route != 'anim')
                                <option value="{{ route('anim') }}">Игра</option>
                            @endif
                            @if(Auth::user()->admin == 1)
                                <option value="{{ route('faces') }}">Админ</option>
                            @endif
                        </select>
                    </div>
                </div>
        ...
        <div class="user--profile">
            ...
            <div class="tab-body">
                ...
                <div class="tab-content">
                    ...
                    <form method="POST" action="{{ route('new.email') }}">
                        @csrf
